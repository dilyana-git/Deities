...
**No SVG filter and no blend mode is charged on the resting atlas.** Two live exceptions, both deliberate and both measured: `.node.tier-1 .core.pip` carries two `drop-shadow()`s on the 12 primaries (and is deliberately scoped to them, though 75 stars now carry the class) (measured at 1.02x — inside the noise, because it is 12 small elements), and `.depth-haze` still screen-blends in the backdrop (free now that the backdrop repaints 10 times a second instead of 60: 59.6 fps blended vs 56.2 unblended vs 60.0 deleted, all within noise — and unlike the limb its blend is *not* cosmetically neutral, at mean 4.08/255 with 11% of pixels moving. Don't "finish the job" on this one).
...
Each node `<g>` stacks: a `glow` halo circle, a gold `sel-halo` bloom circle (invisible until `.selected`/`.route`), a `core` circle, a **frameless** portrait `<image>` on tiers 1–2, and a `node-label` text.

- **Every star keeps its glow, portrait or not — and the reason is worth keeping.** It was briefly taken off the 75 stars that draw a face, on the theory that it was the coloured ring showing through the art's fringe. **It was not.** The ring was `.core` (below), a full-radius family disc with a *hard* edge at exactly 1.0r; removing the glow **sharpened** it, by taking away the falloff that had been blending past that edge. The glow is a smooth gradient running to nothing at 1.5r at 0.07–0.21 opacity — the light the portrait floats in — and it is also the substrate **every** emphasis path animates: the ignition birth flare, the hub breath, the hover swell and the selection nova all write this circle's `r` and `--glow-base`. Take it off a node and all four silently become no-ops on an empty selection, which is a lot of the map's feedback gone for a ring that was never this element's. It is also where family colour lives on tiers 1–2, now that their cores are near-white pips.
  - **`flareTerminal` still guards for a missing glow before setting `_flareId`.** Dead insurance today, kept because the failure is silent and permanent: it clears that id in the transition's `end`, and a transition on an empty selection never ends — the id would latch and `stepAmbient` would stand aside from that node's radius for the rest of the session.

- **The core is a disc where no portrait is drawn and a *pip* where one is — `hasPortrait`, the same rule as the glow.** `coreRadius(n, r)` returns `r` for the tail — there the disc IS the mark, so it carries a lot of the family colour and is the most legible thing on the field — but `r * CORE_R` (0.2) wherever art is drawn, near-white/gold (`#fff7e0`, `.core.pip`, the same `.cl-core.hero` mark the constellation stages use for a hero star). At 30–44px a full-radius pale disc is a lit *plate*: its hard circular edge reads as a bezel around the face however softly the portrait's own mask fades, and it lifts the backdrop the figure has to stand on. Shrunk to a point, the gradient halo carries the light outward and the figure floats *in* that light — a star that happens to have a face in it. The pip stays **under** the portrait (the light the face is lit by, not a glint punched over it), so on most primaries it burns invisibly; what it is really for is the primaries with no artwork, which have to read as stars on their own. It also drops the primary's prom opacity ramp — the tier's radius ramp already carries that difference into the pip.
  - **This used to read `n.tier === 1`, and that was the second coloured ring.** It was written when secondaries showed no portrait *at rest*; they always do now, so 63 tier-2 stars were drawing a full-radius family disc **under** a portrait whose mask holds full alpha only to `0.55 × IMG_SCALE = 0.80r`. The disc showed through the art's fringe from 0.80r out to a **hard edge at exactly 1.0r** — ember-red hooped around every monster's face, teal around every sea god's. Removing the glow *sharpened* it, by taking away the soft falloff that had been blending past that edge. Keying both on `hasPortrait` is what makes the two fixes one rule instead of two that can drift.
  - **The pip's bloom stays tier 1's** (`.node.tier-1 .core.pip`). Every portrait star wears the `.pip` class now, but the two `drop-shadow()`s are two per-pixel convolutions, and widening that rule from 12 elements to 75 is the standing filter cost the glow gradient and the ambient ticker exist to keep off this field. A secondary's pip sits under an opaque portrait and is not there to be seen.
...
- **The foot is five `Row`s in two registers.** Every row shares one skeleton — a Cinzel label, dot-joined values, truncated with the remainder spelled out in **arabic** (`+14 more`, so the count reads as information rather than roman decoration) as a **toggle that opens the rest in place**, no chips/borders/bullets — but they are split into two groups, because five identical italic lines of the same size read as a receipt rather than as information. `.col-rows-attr` holds **Domains, Symbols, Myths** (attributes: italic, 16px, dim, 5px gaps — one tight block); `.col-rows-nav` holds **Bonds** and **Tales** (navigation: upright, 17px, a step brighter, gold hover underline on `.col-link`). **Upright type is the click affordance — don't set a non-navigable row in it.** That is why **Myths stays with the attributes** despite reading as titles: a myth name is a keyword with no target behind it. **Bonds** are the top-degree neighbours and call `onNavigate(id)`; **Tales** come from the module-level `_talesByFig` index (walks `TOURS` once, mapping node id → tales with the **first** beat index where the figure takes the stage) and call `onOpenTale(tourId, beat)` → the Guided Sky overlay opens *at that chapter*. Myths use `emphasize={_famousMyths}` (shared by 3+ figures) → `.col-em`.
  - **`+N more` is a disclosure button, not a caption.** It was a dead `<span>` naming a count the reader could see and not reach — and on Bonds, where the list is the figure's whole adjacency, that stranded everything past the third: Zeus showed `+22 more` with no way to any of them. It now toggles the row open and back (`less`), so the cap is a default rather than a ceiling. It keeps the quiet italic in **both** registers — it is not one of the values, so it must not borrow the navigable row's upright type — and carries the `.col-link` hover/focus underline in gold at a lower amplitude (`#b9ad92` / .4 against the links' `#cdb88a` / .55). **Its base colour is the readability floor's `#8a94a6`, not a dimmer grey**: a control may not read fainter than the values it sits among. `.col-type` is `overflow-y: auto` with a hidden scrollbar, so an expanded Zeus (25 bonds, the dataset's longest) scrolls the measure at 720px rather than pushing the CTA off; state lives in `Row` and needs no reset, because `PanelContent` is keyed by node id. **Expanding does not widen the warm**: `warmNext` still warms `BOND_MAX` heroes, since the extra rows are names, not plates.
...
- **TODAY on the ecliptic.** A single pulsing gold mark at the longitude the current date maps to, so the caption's `Aug 23 – Sep 22` reads as a position on the wheel rather than just a label. `zodiacPlace()` in `zodiac.js` derives it by **parsing the `dates` strings themselves** — one set of dates, so the mark can never disagree with the caption. The signs are stored in date order with contiguous ranges, and `ZodiacSphere` gives sign `i` the 30° around `(i/12)·2π`, so a date lands at `(i − 0.5 + fraction through its range)` twelfths of a turn: a range's **first** day sits on the sign's leading edge, not its centre. Capricorn straddles New Year, so the lookup tries this year's window and last year's. Verified: every day of a leap and a common year maps to exactly one sign, longitude advances monotonically over a full zodiac year, and on screen a sign's first day draws ~80px right of its centre, its last day ~85px left (longitude climbs *leftward* on the front face).
  - It takes the **ecliptic's gold, not the sign's element accent** — it belongs to the ring it sits on, not to whichever figure is standing over it this month. What separates it from the hundreds of stars around it is that it *breathes*: a still dot at this scale is another star, and sizing it up to read as a marker would put it in competition with the figures. The pulse is opacity only.
  - **It is face-down on open.** The view starts on Aries and today is wherever it is, so the mark is usually on the back of the sphere until the carousel comes round. That is the same depth gate everything else here obeys; if it should greet the viewer instead, the thing to change is `ZodiacSky`'s initial `selected`, not the marker.
...
| `.node .glow` (+ `.hub-breath`) | The family-coloured halo, on every star — and on tiers 1–2 it is where family colour lives, their cores being near-white pips. Its `opacity` — and a hub's `r` — are written by the ambient ticker through inline `style`, so anything that wants to brighten a glow writes `--glow-base` instead and lets the ticker read it |
...
| `.col-row` + `.col-row-k` / `.col-row-v` | Foot rows: Cinzel label → dot-joined values (`.col-sep`, `.col-em`), with `.col-more` the button that opens the truncated remainder in place |
...
| `.core` (+ `.pip`) | The star mark — a family-coloured disc **only where no portrait is drawn** (the tail); everywhere art is drawn a near-white `.pip` at 0.2r, so the family colour can never hoop the portrait through its faded fringe. The tight bloom is tier 1's alone (`.node.tier-1 .core.pip`) — 75 stars carry the class, but two `drop-shadow()`s on all of them is a standing filter cost, and a secondary's pip sits under an opaque portrait |
