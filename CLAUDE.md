...
| `.col-figure` > `.col-figure-in` > `.col-figure-veil` > `.col-figure-img` | The figure: zone + ambient bob / art-shaped box (`--fig-ar`) + head-foot mask / left-right mask / radial vignette. `.ready` fades the img in |
| `.col-type` | The measure — a `container-type: inline-size` container so `.col-name` can size in `cqw` |
| `.col-name` / `.col-epithet` / `.col-prose` (+ `.clamped`) | Display name (capped 96px, `--name-cq`) / gold epithet / opening prose with drop-cap and overflow fade |
...
| `.col-enter` (+ `.col-enter-rule`) / `.col-close` | ENTER THE STORY launcher / bare ✕ |
