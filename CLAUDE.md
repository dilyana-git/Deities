...
Each node `<g>` stacks: a `glow` halo circle **on art-less stars only**, a gold `sel-halo` bloom circle (invisible until `.selected`/`.route`), a `core` circle, a **frameless** portrait `<image>` on tiers 1–2, and a `node-label` text.

- **The coloured halo and the portrait are mutually exclusive — `hasPortrait` is the one rule that keeps them so.** `GLOW_R` is 1.5 and `IMG_SCALE` 1.45, so the glow disc is very nearly the portrait's own size; the portrait mask holds full alpha only to **55%** of its box and is transparent by 100%. The category colour therefore came up through the art's entire outer fringe as a coral/violet/teal ring standing *inside* the figure — the frameless portrait's whole point is that it has no ring, and this was that ring reintroduced as light. So the 75 stars that draw a face carry no `.glow`; the 64 tail dots, which draw no `<image>`, keep it, because there the halo **is** the star. A tier 1/2 figure added with no art keeps it too (none today — every primary and secondary has a plate — but one would otherwise be a 30–44px void). A portrait star is lit by the art itself, the `.core` pip under it, and the gold `.sel-halo` behind it.
  - **What went with it, deliberately:** those 75 stars no longer take the ignition birth flare, the hub breath, the hover swell or the selection nova, since every one of those animates `.glow`. Each is a no-op on an empty selection, so nothing throws — but hover now reads through the field dimming and the portrait's lit-state scale, and selection through the gold bloom.
  - **`flareTerminal` bails when there is no glow, before setting `_flareId`.** It clears that id in the transition's `end`, and a transition on an empty selection never ends — the id would latch and `stepAmbient` would stand aside from that node's radius for the rest of the session. `growLineage` walks to a *leaf*, which is usually a tail dot and still flares; when the heir has a face, `layResidue` still inks the thread, so the arrival reads.
...
- **The glow's softness is a gradient, not a filter.** (On the art-less stars that still have one — see above.) It fills with a per-category `node-glow-{cat}` radial gradient (1 → 0.58 → 0.19 → 0 alpha) and carries **no `#glow` filter**. A flat fill softened by the shared 3.2px blur is a glow on a 3.4px tail dot and a hard-edged disc on a 44px primary, where 3px of feather is 7% of the radius — and the filter was conditional on `prom > 0.65`, so hubs just under it (Hera, 0.638) drew a crisp coloured circle ~140px across. Because the gradient is `objectBoundingBox`, one falloff holds from the tail dot to a nova'd primary. Don't put a flat `fill` back on `.glow`.
...
| `.node .glow` (+ `.hub-breath`) | The family-coloured halo, on **art-less stars only** (tail dots, and any tier 1/2 figure with no plate) — a star with a face on it has none, or the colour would ring the portrait through its own faded fringe. Its `opacity` — and a hub's `r` — are written by the ambient ticker through inline `style`, so anything that wants to brighten a glow writes `--glow-base` instead and lets the ticker read it |
