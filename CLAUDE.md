...
└── data/
    ├── mythology.js               # nodes[] (139 nodes) + links[] (258 links) — the entire dataset
...
│   ├── {id}-node.webp             # 192px head crop — tier-2 stars, GuidedSky orbs
...
> **Legacy note:** an earlier React-driven implementation (`GraphCanvas.jsx`, `DeityModal.jsx`, `FilterBar`, `LegendBar`, `NodeTooltip`, `Lightbox`, the `hooks/` and `utils/` dirs) was removed once `SkyGraph` + `DetailPanel` superseded it. A later pass removed `StoryOrbit.jsx` (the reading column took over every story door), `ConstellationStage.jsx` and its `constellations.js` specs, `archetypeMap.js`, the in-graph TourBar, and the dead `.so-*`/`.cl-*`/wireframe CSS. Don't reintroduce per-tick React state for the graph — see the architecture section.
...
Portraits are resolved purely from `id` by convention (no `head_image`/`full_image` fields are read by the current code). `degree` and `prom` (renown) are computed at runtime, not stored. The `jungian_archetype` and `image_prompt` fields were removed once nothing read them — ~30KB of the dataset that shipped in every bundle; the prompts are archived in `docs/portrait-prompts-archive.md`.
...
**The sky sleeps when nothing can see it.** A hidden tab and a full-screen overlay are the same condition, so one switch (`applyDormancy`) owns the `paused` class, and `setDormant(v)` on the imperative API is how `App` reports that GuidedSky / ZodiacSky are up. Each of those is `position: fixed; inset: 0` over an **opaque** ground at z-index 1000, so everything the map draws under one is unseen: ~63 shimmering stars and 17 blurred flare blooms, 12 dash-flow links, the top 6 hubs' breath, the drifting haze, the ambient comet — and above all the two float layers, which transform the whole graph group (139 nodes, 258 edges) and so re-rasterize it every frame. It is charged **twice** wherever an overlay holds a `backdrop-filter` panel over that region, because a backdrop that changes every frame can never be cached — the blur is recomputed for each one.
...
- **Panel width is `--panel-w`** (`:root` in `index.css`, `clamp(520px, 67vw, 1180px)`) — the single source of truth, alongside `--panel-wash`.
...
- **The hero is static; the chapters are not.** One face holds the centre for the whole tour while the narration travels around it — which is why the tour needs a `hero` in its data rather than reusing the beat’s figure.
...
- **Tours** — the full-screen GuidedSky overlay (cinematic, autoplay), built from `tours.js`. The older in-graph TourBar was removed.
...
shortcutsOpen    — keyboard-shortcuts popover visible
searchOpen       — search field expanded
```
...
- **No Cinzel label below 12px, and no reading text below 14px.** Cinzel renders
  ...
  | register | size | where |
  |---|---|---|
  | Cinzel eyebrow / row key / small-caps button | **12 – 13.5px** | `.gsr-kicker`, `.col-row-k`, `S.hbtn`, the legend heads |
  | Cinzel title inside a panel | **15 – 17.5px** | `.gsr-tale-t` |
  ...
  | body a reader actually reads | **16.5 – 20.5px** | `.col-prose`, `.gsr-beat`, `.gsr-full p`, `.gs-narration` |
...
- **Tracking comes down as size goes up.** .26–.4em was holding a 10.5px label
  ...
  .02–.06em with it (`.col-row-k` .26 → .2, `.gsr-beat-head
...
| `.story-text::first-letter` | Drop-cap on the GuidedSky reader prose |
...
| `.gs-root` / `.gs-caption` / `.gs-name` | Shared cinematic-overlay chrome — now ZodiacSky's only (GuidedSky wears `.gsr-*`) |
